			...
			if (Input.touchCount>0 && !finish) {
			  ...
			  if(touch.phase==TouchPhase.Stationary){
			    ...
			    if (touch.position.x<Screen.width/2) {
				    FrontLeftWheel.motorTorque = EngineTorque / GearRatio[CurrentGear]*-0.75 ;
					FrontRightWheel.motorTorque = EngineTorque / GearRatio[CurrentGear]*-0.75 ;
			    }
			       else if(touch.position.x>Screen.width/2){
        			
					FrontLeftWheel.motorTorque = EngineTorque / GearRatio[CurrentGear]*2 ;
					FrontRightWheel.motorTorque = EngineTorque / GearRatio[CurrentGear]*2 ;
			       }
...
//
	if(inTheAir){	
	  ...
	  if (Mathf.Abs(iPx) > moveThreshold){
	    ...
	    transform.Rotate(Vector3.right * tiltSides * 8);
